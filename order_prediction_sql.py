...
def load_data(server, database, username=None, password=None):
    ...
    try:
        ...
        WHERE 
            k.ORDER_JRNL_TYP_TXT in ('Release','Comment','First Approval','Second Approval') 
            and d.ORDER_STATUS_CD in ('P','C','I') and d.ORDER_SCORE_FCT<>-2
		order by 1 desc
        """
...
def prepare_prompt(selected_record, df):
    ...
    prompt = f"""
    You are provided with historical order trend data and corresponding journal comments (if available), along with a new order and its trend data.

    Base Order Data (some records may or may not have comments):
    {json.dumps(other_records_data, indent=2)}

    New Order Number: {selected_record.ORDERNUMBER}

    New Order Trend Data:
    {json.dumps(selected_trends, indent=2)}

    First, analyze the base order data to understand how trend patterns relate to journal comments when comments are available. Identify any consistent patterns or insights.

    Then, evaluate the new order's trend data in the context of your analysis. If similar patterns are found in the base data, use those to guide the predicted comment. If no clear matches are available, generate a reasonable and specific comment based solely on the trend data and your understanding of typical comment styles.

    Return ONLY a valid JSON object with exactly these two fields:
    1. "predicted_comment": A concise, specific journal comment for the new order, written in the similar style and terminology as seen in the base comments and ignore if any Individual names are included.
    2. "reason": A brief explanation of why this comment was predicted, referencing similar past orders if applicable, or explaining how the trend data informed the prediction.
